...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\release\package-windows.ps1
```
...
The Windows package script writes
`dist/summerpoem-v<VERSION>-windows-x64.zip` and prints its SHA-256.
...
For an Ubuntu seed node or Linux miner:

```bash
bash scripts/release/package-linux.sh
```

The Linux package script writes
`dist/summerpoem-v<VERSION>-linux-x64.tar.gz` and prints its SHA-256. A seed
node does not need CUDA or a GPU; GPU-capable Linux miner builds need the CUDA
toolkit available during the build.
...
3. Upload the `.zip` as a release asset.
   Upload the Linux `.tar.gz` too if publishing a Linux build.
